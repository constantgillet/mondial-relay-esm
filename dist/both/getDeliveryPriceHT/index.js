 ...
 * @experimental
 * */
export default function getDeliveryPriceHT(weightInGrams, destinationCountryCode) {
    const priceTable = {
    ...
    if (!supportedCountries.has(destinationCountryCode)) {
        throw new Error(`The destination country code ${destinationCountryCode} is not supported. Supported country codes are: ${Array.from(supportedCountries).join(', ')}`);
    }
    ...
    throw new Error(`The weight ${weightInGrams} is not supported. Supported weights are: ${Object.keys(priceTable).join(', ')}`);
}
